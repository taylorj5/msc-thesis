 ...
 * \param collFlag	array to store whether a particle has been involved in a collision
 */
__global__ void collisions(curandState *state, int* cellX, int* cellY, double* positionX, double* positionY, double* velocityX, double* velocityY, int* collFlag) {
	int idx = blockDim.x * blockIdx.x + threadIdx.x;
	int tid = threadIdx.x;
	int n = npC;
	double p, r;
	...
	sdata[5*tid+4]	= (idx < n) ? collFlag[idx] : 0;
	__syncthreads();
	...
	if (idx < n) {
		...
		for (int i=0; i<npC; i++) {
			//ensure target is different particle and hasn't been involved in a collision in the same timestep
			if (idx!=i && collFlag[i]==0) {
				//we need only consider other particles in the same cell
				if (cellX[idx] == cellX[i] && cellY[idx] == cellY[i]) {
					tmpX = positionX[i];
					tmpY = positionY[i];		
		...
		//we only wish to calculate each pair once
		collFlag[target] = -1;
...
double picFloatGpu(double* positionX, double *positionY, double* velocityX, double* velocityY, double *xResult, double *yResult, std::shared_ptr<Params> p) {
	...
	cudaMalloc((void**) &collFlags, 		np*sizeof(int));	//memory for flags in collision step

	//filling collFlags with zeroes
	cudaMemset(collFlags, 0, np*sizeof(int));
	...
	while (t < p->tmax) {
		...
		//simulate collisions
		collisions<<<dimGridParticles,dimBlock,smem5,stream[0]>>>(states,currentCellXGpu, currentCellYGpu, positionXGpu, positionYGpu, velocityXGpu, velocityYGpu, collFlags);

		//reset collFlags to zero
		cudaMemset(collFlags, 0, nx*ny*sizeof(int));
