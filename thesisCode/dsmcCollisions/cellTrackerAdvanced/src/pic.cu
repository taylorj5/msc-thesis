 ...
 */
__global__ void cellTrackerSetup(int* cellX, int* cellY, cudaSurfaceObject_t surf) {
	int idx = blockDim.x * blockIdx.x + threadIdx.x;
	int tid = threadIdx.x;
	int n = nxC*nyC;
	...
	if (idx < n) {
		int i = idx/nyC; //cell coordinate in x direction
		int j = idx%nyC; //cell coordinate in y direction
		int row = (j + i*nyC); //first coordinate in surface array
		int col = 0;
...
/**
 * \brief Kernel for grid cells to determine which particles are migrating to different cells
 * Identifies relevant particles before pushing them to the end of the array to be moved by a different kernel
 * The end result should be the end of the array containing all particles that have to be migrated
 *
 * \param cellArray	array to store particles residing in each cell
 * \param cellX		array containing current cell in x-direction of particle
 * \param cellY		array containing current cell in y-direction of particle
 */
__global__ void cellTrackerMigrationSetup(int* cellX, int* cellY, cudaSurfaceObject_t surf) {
	int idx = blockDim.x * blockIdx.x + threadIdx.x;
	int tid = threadIdx.x;
	int n = nxC*nyC;
	...
	if (idx < n) {
		...
		//loop over subarray from end to start
		for (int k=cellMaxC-1; k>=0; k--) {
			//read particle index from surface memory
			int index;
			surf2Dread(&index, surf, row*sizeof(int), k);
			__syncthreads();

			//when we come across a particle (signified by array entry != -1) we check if it should be migrated
			if (index != -1) {
				//obtain current cell of that particle
				int tmpX = cellX[index];
				int tmpY = cellY[index];
				//if particle is no longer in the same cell it must be moved and replaced
				if (tmpX != i && tmpY != j){
					//move particle to be migrated to the end of the row
					surf2Dwrite(index, surf, row*sizeof(int), cellMaxC-1-migrated, cudaBoundaryModeTrap);	
					migrated++;
	
					//move last particle in array to spot vacated by migrating particle
					int index2;
					for (int m=cellMaxC-1; m>0; m--) {
						surf2Dread(&index2, surf, row*sizeof(int), m);
						//when we get to the first non-empty index, this particle will be the last particle in the cell
						//it must be noted that due to surface caching, the particles written to the end will not be visible to this kernel
						//which is why we may loop over the index in reverse-order without worry of selecting the incorrect particle
						if (index != -1 && index != k) {
							//move the last particle to the newly empty slot
							surf2Dwrite(index2, surf, row*sizeof(int), k, cudaBoundaryModeTrap);
							//vacate the spot previously occupied by the last particle
							surf2Dwrite(-1, surf, row*sizeof(int), m, cudaBoundaryModeTrap);
						}	
					}
				}
			}
...
/**
 * \brief Kernel for grid cells to receive migrating particles from their neighbours
 *
 * \param cellArray	array to store particles residing in each cell
 * \param cellX		array containing current cell in x-direction of particle
 * \param cellY		array containing current cell in y-direction of particle
 */
__global__ void cellTrackerMigration(int* cellX, int* cellY, cudaSurfaceObject_t surf) {
	int idx = blockDim.x * blockIdx.x + threadIdx.x;
	int tid = threadIdx.x;
	int n = nxC*nyC;
	int x = nxC;
	...
	if (idx < n) {
		int i = idx/y; //cell coordinate in x direction
		int j = idx%y; //cell coordinate in y direction
		int row = (j + i*y); //first coordinate in surface array
		...
		int topLeftN=0, topRightN=0, bottomLeftN=0, bottomRightN=0;
		//Finding neighbours of cell with periodic boundary conditions
		//allows for all 8 special cases and the general case
		//at times, algebra is made simpler to enhance instruction-level parallelism
		if (i==0 && j==0) {
			leftN 	= (x-1)*y;
			rightN 	= y;
			topN 	= 1;
		...
		//Identifying first vacant space in the current cell
		for (k=0; k<cellMaxC; k++) {
			int index;
			surf2Dread(&index, surf, row*sizeof(int), k);
			if (index == -1) {
				//we store the index of the first vacant space in pos
				pos = k;
				break;
			}	
		}		

		//Checking for particles to be moved from left neighbour
		for (k = cellMaxC-1; k>=0; k--) {
			int index2;
			surf2Dread(&index2, surf, leftN*sizeof(int), k);
			__syncthreads();
			
			//if the position has no particle then we have finished iterating
			if (index2 == -1) {
				break;
			}
			//if there is a particle and it should be in this cell, we move it to the current cell and replace it with -1
			if (cellX[index2] == i && cellY[index2] == j) {
				surf2Dwrite(index2, surf, row*sizeof(int), pos, cudaBoundaryModeTrap);	
				surf2Dwrite(-1, surf, leftN*sizeof(int), k, cudaBoundaryModeTrap);	
				pos++;
			}			
		}
		//Checking for particles to be moved from right neighbour
		for (k = cellMaxC-1; k>=0; k--) {
			int index2;
			surf2Dread(&index2, surf, rightN*sizeof(int), k);
			__syncthreads();
			...
			if (cellX[index2] == i && cellY[index2] == j) {
				surf2Dwrite(index2, surf, row*sizeof(int), pos, cudaBoundaryModeTrap);	
				surf2Dwrite(-1, surf, rightN*sizeof(int), k, cudaBoundaryModeTrap);	
				pos++;
			}			
		}
		//Checking for particles to be moved from top neighbour
		for (k = cellMaxC-1; k>=0; k--) {
			int index2;
			surf2Dread(&index2, surf, topN*sizeof(int), k);
			__syncthreads();
			...
			if (cellX[index2] == i && cellY[index2] == j) {
				surf2Dwrite(index2, surf, row*sizeof(int), pos, cudaBoundaryModeTrap);	
				surf2Dwrite(-1, surf, topN*sizeof(int), k, cudaBoundaryModeTrap);	
				pos++;
			}			
		}
		//Checking for particles to be moved from bottom neighbour
		for (k = cellMaxC-1; k>=0; k--) {
			int index2;
			surf2Dread(&index2, surf, bottomN*sizeof(int), k);
			__syncthreads();
			...
			if (cellX[index2] == i && cellY[index2] == j) {
				surf2Dwrite(index2, surf, row*sizeof(int), pos, cudaBoundaryModeTrap);	
				surf2Dwrite(-1, surf, bottomN*sizeof(int), k, cudaBoundaryModeTrap);	
				pos++;
			}			
		}
		//Checking for particles to be moved from top left neighbour
		for (k = cellMaxC-1; k>=0; k--) {
			int index2;
			surf2Dread(&index2, surf, topLeftN*sizeof(int), k);
			__syncthreads();
			...
			if (cellX[index2] == i && cellY[index2] == j) {
				surf2Dwrite(index2, surf, row*sizeof(int), pos, cudaBoundaryModeTrap);	
				surf2Dwrite(-1, surf, topLeftN*sizeof(int), k, cudaBoundaryModeTrap);	
				pos++;
			}			
		}
		//Checking for particles to be moved from top right neighbour
		for (k = cellMaxC-1; k>=0; k--) {
			int index2;
			surf2Dread(&index2, surf, topRightN*sizeof(int), k);
			__syncthreads();
			...
			if (cellX[index2] == i && cellY[index2] == j) {
				surf2Dwrite(index2, surf, row*sizeof(int), pos, cudaBoundaryModeTrap);	
				surf2Dwrite(-1, surf, topRightN*sizeof(int), k, cudaBoundaryModeTrap);	
				pos++;
			}			
		}
		//Checking for particles to be moved from bottom right neighbour
		for (k = cellMaxC-1; k>=0; k--) {
			int index2;
			surf2Dread(&index2, surf, bottomRightN*sizeof(int), k);
			__syncthreads();
			...
			if (cellX[index2] == i && cellY[index2] == j) {
				surf2Dwrite(index2, surf, row*sizeof(int), pos, cudaBoundaryModeTrap);	
				surf2Dwrite(-1, surf, bottomRightN*sizeof(int), k, cudaBoundaryModeTrap);	
				pos++;
			}			
		}
		//Checking for particles to be moved from bottom left neighbour
		for (k = cellMaxC-1; k>=0; k--) {
			int index2;
			surf2Dread(&index2, surf, bottomLeftN*sizeof(int), k);
			__syncthreads();
			...
			if (cellX[index2] == i && cellY[index2] == j) {
				surf2Dwrite(index2, surf, row*sizeof(int), pos, cudaBoundaryModeTrap);	
				surf2Dwrite(-1, surf, bottomLeftN*sizeof(int), k, cudaBoundaryModeTrap);	
				pos++;
			}			
		}
	}
...
double picFloatGpu(double* positionX, double *positionY, double* velocityX, double* velocityY, double *xResult, double *yResult, std::shared_ptr<Params> p) {
	//Choosing which device to use. Choices for cuda01 are listed below:
	...
	const int cellMax = (np > nx*ny) ? (np/(nx*ny))*6 : 6;
	...
	//dim3 dimBlockCells(p->blockSize);
	...
	//due to the collisions, it must be updated as part of the particle push step rather than at the start of the iteration
	...
	while (t < p->tmax) {
		...
		//update cellTracker array
		if (t==0){
			//at time t=0 we need to build the cellTracker array from scratch
			cellTrackerSetup<<<dimGridCells,dimBlockCells,smem2,stream[0]>>>(currentCellXGpu, currentCellYGpu, surface);
		} else {
			//otherwise we migrate particles between different cells in the cellTracker array
			cellTrackerMigrationSetup<<<dimGridCells,dimBlockCells,smem2,stream[0]>>>(currentCellXGpu, currentCellYGpu, surface);
			cudaDeviceSynchronize();
			cellTrackerMigration<<<dimGridCells,dimBlockCells,smem2,stream[0]>>>(currentCellXGpu, currentCellYGpu, surface);
		}
		cudaDeviceSynchronize();
