 ...
 */
__global__ void cellTracking(int* cellX, int* cellY, int *cellTracker) {
	int idx = blockDim.x * blockIdx.x + threadIdx.x;
	int tid = threadIdx.x;
	int n = nxC*nyC;
...
__global__ void collisions2(curandState *state, int* cellX, int* cellY, double* positionX, double* positionY, double* velocityX, double* velocityY, int *cellTracker) {
	int idx = blockDim.x * blockIdx.x + threadIdx.x;
	int tid = threadIdx.x;
	int n = npC;
	double p, r;
	...
	if (idx < n) {
		...
		int pos = cellMaxC * (i*nyC + j);

		//we loop through all particles in the same cell to create collision pairs
		for (int k=pos; k<pos+cellMaxC; k++) {
			int index = cellTracker[k];
			if (index == -1) {
				break;
			}
...
double picFloatGpu(double* positionX, double *positionY, double* velocityX, double* velocityY, double *xResult, double *yResult, std::shared_ptr<Params> p) {
	...
	cudaMalloc((void**) &cellTracker,		cellMax*ng*sizeof(int));//memory for keeping track of which cell particles are in

	//filling cellTracker with -1
	cudaMemset(cellTracker, -1, np*ng*sizeof(int));
	...
	while (t < p->tmax) {
		...
		cellTracking<<<dimGridCells,dimBlock,smem2,stream[0]>>>(currentCellXGpu, currentCellYGpu, cellTracker);
		cudaDeviceSynchronize();

		//simulate collisions
		collisions2<<<dimGridParticles,dimBlock,smem5,stream[0]>>>(states,currentCellXGpu, currentCellYGpu, positionXGpu, positionYGpu, velocityXGpu, velocityYGpu, cellTracker);

		cudaDeviceSynchronize();	

		//reset collFlags to zero
		cudaMemsetAsync(collFlags, 0, nx*ny*sizeof(int), stream[0]);
	
		//reset cellTracker to -1
		cudaMemsetAsync(cellTracker, -1, cellMax*ng*sizeof(int), stream[1]);
