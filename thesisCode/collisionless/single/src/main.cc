...
void checkResults(std::vector<Particle> &particles, float* xGpu, float *yGpu);

//cuda wrapper functions
extern float picFloatGpu(float *positionX, float *positionY, float *velocityX, float *velocityY, float *xResult, float *yResult, std::shared_ptr<Params> p);
...
int main(int argc, char **argv) {
	...
	float initTime, timeCpu, timeGpu;
	...
	p->dx = (float) p->xDim/p->nx;
	p->dy = (float) p->yDim/p->ny;
	p->area = p->dx * p->dy;
	...
	float *positionX= new float[p->numParticles];
	float *positionY= new float[p->numParticles];
	float *velocityX= new float[p->numParticles];
	float *velocityY= new float[p->numParticles];
	...
	if (cpu) {
		...
		while (t < p->tmax) {
			...
			if (t == 0.0) {
				for (auto j=0; j<p->nx+2; j++) {
					for (auto k=0; k<p->ny+2; k++) {
						//std::cout << grid.chargeDensity[j*grid.y+k] << "\t";
					}
					//std::cout << '\n';
				}
			}
	...
	if (gpu) {
		float *positionXGpu = new float[p->numParticles];
		float *positionYGpu = new float[p->numParticles];
...
void initialiseParticles(std::vector<Particle> &particles, std::shared_ptr<Params> p) {
	...
	float xCoord, yCoord;
	float xOffset = 0.5*(float)(p->xDim/p->nx);
	float yOffset = 0.5*(float)(p->yDim/p->ny);
	Point tmp;
	...
	for (auto i=0; i<p->numParticlesY; i++) {
		...
		yCoord = ((float)p->yDim/p->numParticlesY)*(float)i + yOffset;
		for (auto j=0; j<p->numParticlesX; j++) {
			xCoord = ((float)p->xDim/p->numParticlesX)*(float)j + xOffset;
	...
	//add drift velocity 
	float driftSumX = 0.0, driftSumY = 0.0;
	for (std::vector<Particle>::iterator it = particles.begin(); it!=particles.end(); ++it) {
		tmp = it->getVelocity();
		driftSumX += tmp.x;
		driftSumY += tmp.y;
	...
	float var = 1.0/(float)p->numParticles;
 ...
 * \param xGpu		float array containing x-coordinates from the GPU
 * \param yGpu		float array containing y-coordinates from the GPU
 */
void checkResults(std::vector<Particle> &particles, float* xGpu, float *yGpu) {
	int i = 0;
