...
void Grid::chargeAssignment(const std::vector<Particle> &particles) {
	...
	for (std::vector<Particle>::const_iterator it = particles.begin(); it!=particles.end(); ++it) {
		...
		float area1 = (it->position.x - it->currentCell.x*p->dx) * (it->position.y - it->currentCell.y*p->dy)/totalArea;
		float area2 = ((it->currentCell.x*p->dx + p->dx) - it->position.x) * (it->position.y - it->currentCell.y*p->dy)/totalArea; 
		float area3 = (it->position.x - it->currentCell.x*p->dx) * ((it->currentCell.y*p->dy + p->dy) - it->position.y)/totalArea;
		float area4 = ((it->currentCell.x*p->dx + p->dx) - it->position.x) * ((it->currentCell.y*p->dy + p->dy) - it->position.y)/totalArea;
