...
void Particle::forceCalculation(Grid& grid, std::shared_ptr<Params> p) {
	...
	float totalArea = p->area;
	float area1 = ((position.x - currentCell.x*p->dx) * (position.y - currentCell.y*p->dy))/totalArea;
	float area2 = (((currentCell.x*p->dx + p->dx) - position.x) * (position.y - currentCell.y*p->dy))/totalArea; 
	float area3 = ((position.x - currentCell.x*p->dx) * ((currentCell.y*p->dy + p->dy) - position.y))/totalArea;
	float area4 = (((currentCell.x*p->dx + p->dx) - position.x) * ((currentCell.y*p->dy + p->dy) - position.y))/totalArea;

	float qm = p->electronCharge/p->mass;
...
void Particle::positionUpdate(std::shared_ptr<Params> p) {
	...
	float vxh = velocity.x + 0.5*dt*forceOld.x;
	float vyh = velocity.y + 0.5*dt*forceOld.y;
