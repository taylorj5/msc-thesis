...
void fftPoissonSolver(Grid& grid, std::shared_ptr<Params> p) {
	...
	const float norm = 1.0/(p->nx*p->ny);	//normalisation factor for output
	...
	fftwf_plan fwrd = fftwf_plan_dft_r2c_2d(nx, ny, in, out, FFTW_MEASURE);
	fftwf_plan bwrd = fftwf_plan_dft_c2r_2d(nx, ny, out, in, FFTW_MEASURE);
...
//		std::cout << '\n';
	...
	fftwf_destroy_plan(fwrd);
	fftwf_destroy_plan(bwrd);
